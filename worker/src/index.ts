...
app.post('/api/write', async c => { try { const patch = await c.req.json<StatePatch>(); state = applyPatch(state, patch); return c.json({ ok: true, state }); } catch { return c.json({ ok: false, error: 'invalid JSON patch' }, 400); } });
