...
export default function App() {
  ...
  const request = useCallback(async (path, options = {}) => {
    ...
    } catch { throw new Error('Network error. Check the Worker connection and try again.'); }
    let body;
    try { body = await response.json(); } catch { throw new Error(`Worker returned invalid JSON (${response.status}).`); }
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [entries, auth] = await Promise.all([Promise.all(endpoints.map(async path => [path, await request(path)])), request('/auth/status')]);
      setData(Object.fromEntries(entries)); setUser(auth.authenticated ? auth : null); setError('');
    } catch (e) {
  ...
  async function login(event) {
    ...
    try { const result = await request('/auth/login', { method: 'POST', body: JSON.stringify(credentials) }); localStorage.setItem('portal-os-token', result.token); setToken(result.token); setCredentials({ username: '', password: '' }); setError(''); }
    catch (e) { setError(e.message); }
  }
  async function logout() { try { await request('/auth/logout', { method: 'POST' }); } catch { /* local logout remains safe */ } localStorage.removeItem('portal-os-token'); setToken(''); setUser(null); setData({}); }
  ...
  async function toggleUmbrella() {
    ...
    setData(current => ({ ...current, '/umbrella': { mode: next }, '/state': oldState && { ...oldState, umbrella: { ...(oldState.umbrella || {}), mode: next }, apex: { ...(oldState.apex || {}), enforcement: next === 'enabled' } } }));
    setUmbrellaUpdating(true); setError('');
    try { await request('/umbrella', { method: 'POST', body: JSON.stringify({ mode: next }) }); await load(); }
    catch (e) { setData(current => ({ ...current, '/umbrella': { mode: previous }, '/state': oldState })); setError(`Umbrella update failed: ${e.message}`); }
  ...
  const status = data['/status']; const umbrella = data['/umbrella']; const globalState = data['/state']; const enabled = umbrella?.mode === 'enabled';
